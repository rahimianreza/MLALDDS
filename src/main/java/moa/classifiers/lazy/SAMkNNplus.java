...
/*
 * Self-Adaptive k-Nearest Neighbors for Multi-Label Drifting Data Streams
 * This version of the MLSAkNN algorithm is used as the base classifier in MLALDDS and is integrated within the Binary Relevance (BR) framework.
 */
...
public class SAMkNNplus extends AbstractMultiLabelLearner implements MultiLabelClassifier {
	...
	public String getPurposeString() {
		return "Self-Adjusting k Nearest Neighbors (SAMkNN+) for using with Binary Relevance Architecture";
	}
	
	// Setter method to assign value to ModelContext.
	@Override
	public void setModelContext(InstancesHeader context) {
	 ...
	 // resetLearningImpl performs a specific operation.
	@Override
	...
	// Executes trainOnInstanceImpl operation for current instance.
	@Override
	...
	// Predicts the label of a given instance
	@Override
		...
		for(int j = 0; j < numLabels; j++)
		{
			...
			for(int n = 0; n < distancesLocal.length; n++) {
				...
				if(labelInstanceMask[closestNeighbor][j] == 1) {
					...
					// If labelinstance was disabled but it would had been a good predictionLabelSet then reenable
					if(modify && window.get(closestNeighbor).classValue(j) == instance.classValue(j)) {
				...
				if((totalVotes == 1 || totalVotes == 3 || totalVotes == 5 || totalVotes == 7) && enter == true) {
					...
					int kIndex = -1;
					//if performs a specific operation.
					if(totalVotes == 1) {
						kIndex = 0;
					}
	...
	//Getter method to return MetricSums.
	...
	//Getter method to return MetricFromHistory.
	...
	//Computes the Euclidean distance between one sample and a collection of samples in an 1D-array. (Getter method to return 1ToNDistances.)
	...
	// Getter method to return Euclidean distance.
    ...
    // Updates internal state or memory structures.
	...
	// Getter method to return NewWindowSize(bisected size which maximized the metric)
	...
	// nArgMin performs a specific operation (n smallest values (sorted)).
	...
	private double[] getPrediction(int[] nnIndices, List<Instance> instances, int j) {
	 ...
	 //Creates a prediction history from the scratch (TestTrainPredHistory).
	...
	//Creates a prediction history incrementally by using the previous predictions (IncrementalTestTrainPredHistory).
	...
	private List<Integer> getIncrementalTestTrainPredHistory(List<Instance> instances, int startIdx, List<Integer> predictionHistory) {
	 ...
	 // Removes predictions of the largest window size and shifts the remaining ones accordingly (adaptHistories).
